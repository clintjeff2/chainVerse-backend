...
const { authMiddleware, rbacMiddleware } = require('../middlewares/auth');
const { 
...
router.get('/',
  asyncHandler(async (req, res) => {
    ...
    const projection = req.user.role === 'student' 
      ? { title: 1, description: 1, courseId: 1, moduleId: 1, createdAt: 1, 'questions.text': 1 }
      : {};
